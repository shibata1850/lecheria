...
export default function AdminNewsPage() {
  ...
  const handleSave = async () => {
    ...
    if (mode === 'new') {
      ...
      if (form.status === 'published' && editing.status !== 'published') {
  ...
  const handlePublish = async (id: string) => {
    await supabase.from('news').update({ status: 'published', published_at: new Date().toISOString() }).eq('id', id);
    fetchArticles();
  };
  ...
  if (mode === 'new' || mode === 'edit') {
    return (
      <div className="p-8 max-w-3xl mx-auto">
        ...
        <h1 className="font-serif text-2xl font-light text-charcoal mb-8">
          {mode === 'new' ? '新規記事作成' : '記事を編集'}
        </h1>
            ...
            <div key={a.id} className="px-6 py-5 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3">
                  <span className={`text-xs font-sans px-2.5 py-0.5 rounded-full ${STATUS_BADGE[a.status]}`}>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={() => openEdit(a)} className="p-2 text-mid-gray hover:text-charcoal transition-colors" title="編集">
                  <Edit2 className="w-4 h-4" />
                </button>
                {a.status === 'published' ? (
                  <button onClick={() => handleArchive(a.id)} className="p-2 text-mid-gray hover:text-amber-600 transition-colors" title="アーカイブ">
                    <Archive className="w-4 h-4" />
                  </button>
                ) : (
                  <button onClick={() => handlePublish(a.id)} className="p-2 text-mid-gray hover:text-emerald-600 transition-colors" title="公開">
                ...
                {deleteConfirm === a.id ? (
                  ...
                  <button onClick={() => setDeleteConfirm(a.id)} className="p-2 text-mid-gray hover:text-red-600 transition-colors" title="削除">
