...
export default function SubmissionsPage() {
  ...
  const fetchSubmissions = async () => {
    setLoading(true);
    const { data, error } = await supabase.from('submissions').select('*').eq('status', activeTab).order('created_at', { ascending: false });
  ...
  const handleApprove = async (submission: Submission) => {
    ...
    const { error: newsError } = await supabase.from('news').insert({
      slug, title: submission.title, body: submission.body, image_urls: submission.image_urls,
      status: 'published', published_at: new Date().toISOString(), source_submission_id: submission.id,
    });
    if (newsError) { setActionError('公開に失敗しました: ' + newsError.message); setActionLoading(false); return; }
    const { error: updateError } = await supabase.from('submissions').update({ status: 'approved' }).eq('id', submission.id);
    if (updateError) { setActionError('ステータス更新に失敗しました'); setActionLoading(false); return; }
    setSelected(null);
    fetchSubmissions();
  ...
  const handleReject = async (submission: Submission) => {
    if (!rejectNote.trim()) { setActionError('却下理由を入力してください'); return; }
    setActionLoading(true);
    setActionError('');
    const { error } = await supabase.from('submissions').update({ status: 'rejected', admin_note: rejectNote }).eq('id', submission.id);
    if (error) { setActionError('却下処理に失敗しました'); setActionLoading(false); return; }
    setSelected(null);
    setRejectNote('');
  ...
  if (selected) {
    return (
      <div className="p-8 max-w-3xl mx-auto">
        <button onClick={() => setSelected(null)} className="flex items-center gap-2 text-sm font-sans text-mid-gray hover:text-charcoal transition-colors mb-8">
        ...
        <div className="bg-white border border-border p-8 space-y-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`text-xs font-sans px-3 py-1 rounded-full ${STATUS_BADGE[selected.status]}`}>{STATUS_LABEL[selected.status]}</span>
          </div>
              ...
              {showRejectInput ? (
                <div className="space-y-3">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => { setShowRejectInput(false); setRejectNote(''); setActionError(''); }} className="btn-outline text-sm py-2 px-6">キャンセル</button>
                ...
                <div className="flex flex-wrap gap-3">
                  <button onClick={() => handleApprove(selected)} disabled={actionLoading}
                    ...
                    <Check className="w-4 h-4" />承認して公開
                  </button>
        ...
        <div className="bg-white border border-border divide-y divide-border">
          {submissions.map((s) => (
            <button key={s.id} onClick={() => openDetail(s)}
              ...
              <span className={`text-xs font-sans px-2.5 py-1 rounded-full shrink-0 ${STATUS_BADGE[s.status]}`}>{STATUS_LABEL[s.status]}</span>
            </button>
          ))}
        </div>
      )}
