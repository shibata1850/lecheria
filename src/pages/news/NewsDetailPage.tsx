...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    supabase.from('news').select('*').eq('slug', slug).eq('status', 'published').maybeSingle()
    ...
    <>
      <section className="bg-soft-gray py-28 md:py-36">
        <div className="max-w-3xl mx-auto px-6 md:px-10">
          ...
          <p className="text-center text-xs font-sans text-mid-gray tracking-wider mb-4">{formatDate(article.published_at)}</p>
          <h1 className="font-serif text-2xl md:text-3xl font-light text-charcoal text-center leading-relaxed">{article.title}</h1>
        </div>
      </section>
        ...
        <div className="max-w-3xl mx-auto px-6 md:px-10">
          {article.image_urls.length > 0 && (
            <div className="fade-up mb-10">
              <img src={article.image_urls[0]} alt={article.title} className="w-full h-auto object-cover" loading="lazy" />
          ...
          <div className="mt-12 pt-8 border-t border-border fade-up">
            <Link to="/news" className="flex items-center gap-2 text-sm font-sans text-mid-gray hover:text-charcoal transition-colors">
              <ChevronLeft className="w-4 h-4" />お知らせ一覧に戻る
            </Link>
          </div>
        </div>
      </section>
