...
export default function SubmitPage() {
  ...
  const addImages = (files: FileList | null) => {
    if (!files) return;
    const combined = [...images, ...Array.from(files)].slice(0, 3);
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          ...
          <p className="font-serif text-2xl font-light text-charcoal mb-6">ご投稿ありがとうございました</p>
          <p className="text-mid-gray text-sm font-sans leading-loose">
            SOFTDOINGにて内容確認の上、公開いたします。<br />しばらくお待ちください。
          </p>
        </div>
      </div>
    );
  ...
  if (step === 'confirm' && formSnapshot) {
    return (
      <div className="bg-soft-gray py-16 px-6 min-h-screen">
        <div className="max-w-2xl mx-auto">
            ...
            {images.length > 0 && (
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {images.map((img, i) => (
                    <img key={i} src={URL.createObjectURL(img)} alt={`画像${i + 1}`} className="w-24 h-24 object-cover border border-border" />
                  ))}
                </div>
              </div>
            )}
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <button onClick={() => setStep('form')} disabled={uploading} className="btn-outline flex-1 text-center">修正する</button>
            <button onClick={onSubmit} disabled={uploading} className="btn-primary flex-1 text-center">
              {uploading ? '送信中...' : '送信する'}
            </button>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <form onSubmit={handleSubmit(onConfirm)} className="space-y-7">
          <div>
            ...
            <input {...register('submitter_name')} type="text" placeholder="山田 太郎"
            ...
            {errors.submitter_name && <p className="text-red-500 text-xs font-sans mt-1">{errors.submitter_name.message}</p>}
          </div>
          <div>
            <label className="block text-xs font-sans tracking-widest uppercase text-mid-gray mb-2">
              メールアドレス <span className="text-gold">*</span>
            </label>
            <input {...register('submitter_email')} type="email" placeholder="example@email.com"
              className="w-full border border-border bg-white px-4 py-3 text-sm font-sans text-charcoal placeholder-silver focus:outline-none focus:border-gold transition-colors" />
            {errors.submitter_email && <p className="text-red-500 text-xs font-sans mt-1">{errors.submitter_email.message}</p>}
          </div>
          <div>
            <label className="block text-xs font-sans tracking-widest uppercase text-mid-gray mb-2">
            ...
            <div className="relative">
              <input {...register('title')} type="text" placeholder="タイトルを入力してください" maxLength={100}
            ...
            {errors.title && <p className="text-red-500 text-xs font-sans mt-1">{errors.title.message}</p>}
          </div>
          <div>
            ...
            {errors.body && <p className="text-red-500 text-xs font-sans mt-1">{errors.body.message}</p>}
          </div>
          <div>
            <label className="block text-xs font-sans tracking-widest uppercase text-mid-gray mb-2">
            ...
            >
              ...
              <p className="text-sm font-sans text-mid-gray">ドラッグ＆ドロップ、またはクリックして選択</p>
              <p className="text-xs font-sans text-silver mt-1">最大3枚まで</p>
              <input id="image-input" type="file" accept="image/jpeg,image/png,image/webp" multiple className="hidden"
                onChange={(e) => addImages(e.target.files)} />
            </div>
            {imageError && <p className="text-red-500 text-xs font-sans mt-1">{imageError}</p>}
            {images.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-3">
                {images.map((img, i) => (
                  <div key={i} className="relative group">
                    <img src={URL.createObjectURL(img)} alt={`プレビュー${i + 1}`} className="w-24 h-24 object-cover border border-border" />
          ...
          <button type="submit" className="btn-primary w-full text-center">内容を確認する</button>
        </form>
      </div>
    </div>
  );
